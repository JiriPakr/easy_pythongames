...
# heavily inspired by https://github.com/Mekire/Snake/blob/master/snake.py
# copied just for learning

CAPTION = "Snake"
SCREEN_SIZE = (544, 544)
PLAY_RECT = pygame.Rect(16, 16, 512, 512)
CELL = pygame.Rect(0, 0, 16, 16)
BOARD_SIZE = (PLAY_RECT.width//CELL.width, PLAY_RECT.height//CELL.height)
GROWTH_PER_APPLE = 3


COLORS = {"background" : (30, 40, 50), "walls" : pygame.Color("lightslategrey"),
          "snake" : pygame.Color("limegreen"), "head" : pygame.Color("darkgreen"), 
...
KEY_MAPPING = {(pygame.K_LEFT, pygame.K_a) : "left", (pygame.K_RIGHT, pygame.K_d) : "right",
               (pygame.K_UP, pygame.K_w) : "up", (pygame.K_DOWN, pygame.K_s) : "down"}
...
class Apple(object):
    ...
    def respawn(self, obstacles):
        ...
        positon = tuple(random.randrange(BOARD_SIZE[i]) for i in (0,1))
        while positon in obstacles:
            positon = tuple(random.randrange(BOARD_SIZE[i]) for i in (0,1))
        return positon
...
class AnyKey(_Scene):
    ...
    def make_text(self, title):
        """Pre-render text."""
        self.main = FONTS["BIG"].render(title, True, pygame.Color("darkred"))
